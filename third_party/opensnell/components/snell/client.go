...
func (c *Client) DialTCP(ctx context.Context, host string, port uint16) (net.Conn, error) {
	...
	if c.pool != nil {
		for attempts := 0; attempts < 2; attempts++ {
			...
			if werr := c.writeEstablishmentHeader(ctx, conn, host, port, true); werr != nil {
	...
	if err := c.writeEstablishmentHeader(ctx, conn, host, port, c.cfg.Reuse); err != nil {
		return nil, err
	}
	return conn, nil
}

// writeEstablishmentHeader bounds the CONNECT/obfs first write with a socket
// deadline and closes the conn if ctx is cancelled. On success it clears the
// establishment deadline and stops the cancel watcher so the returned
// connection is not tied to the dial context.
func (c *Client) writeEstablishmentHeader(ctx context.Context, conn net.Conn, host string, port uint16, reuse bool) error {
	if conn == nil {
		return errors.New("snell client: nil establishment conn")
	}
	if err := ctx.Err(); err != nil {
		_ = conn.Close()
		return err
	...
	stop := watchEstablishmentCancel(ctx, conn)
	err := WriteHeader(conn, host, port, reuse)
	stop()
...
func watchEstablishmentCancel(ctx context.Context, conn net.Conn) func() {
